        ...
        <div class="modal-body">
          ...
          <form method="post" action="{{route('admin.categories.store')}}" enctype="multipart/form-data">
            ...
            <div class="form-group">
          	...
          	<div class="form-group">
              <label> parent categorey </label>
              <select class="form-control" name="parent_id">
                <option value="">select </option>
                @foreach($categories as $categorey)
                <option
                {{old('parent_id') == $categorey->id ? 'selected' : ''}}
                value="{{$categorey->id}}"
							...
							<div class="container">
								...
								<div class="card-header flex-wrap border-0 pt-6 pb-0">
									<div class="card-title w-100 d-flex justify-content-between">
										<h3 class="card-label float-left">Categories</h3>
  											...
  											<table class="table table-head-custom table-vertical-center" id="kt_advance_table_widget_1">
  												<thead>
  													<tr class="text-center">
  														...
  														<th style="min-width: 400px">name em</th>
  														<th style="min-width: 200px">parent categorey</th>
  														<th class="pr-0 text-right" style="min-width: 150px">أجراء</th>
  													</tr>
  												</thead>
                          @foreach($rows as $row)
                                  ...
                                  <a href="{{files_path($row->image)}}">
  														...
  														<td class="pr-0">
  															<div class="symbol symbol-50 symbol-light mt-1">
  															 {{$row->parent ? $row->parent->name_ar : 'noting'}}
  															</div>
  														</td>
                                ...
                                data-toggle="modal" data-target="#edit-{{$row->id}}">
                                  <span class="svg-icon svg-icon-md svg-icon-primary">
  															...
  															<a href="{{ route('admin.categories.destroy',$row->id) }}"
                                ...
                                <form id="destory-{{$row->id}}"
                                  class="delete"
                                  action="{{ route('admin.categories.destroy',$row->id) }}" method="POST" class="d-none">
                                    @csrf
                                    @method('DELETE')
                                </form>
                            ...
                            <div class="modal-dialog" role="document">
                              <div class="modal-content">
                                ...
                                <div class="text-right modal-body">
                                  ...
                                  <form method="post" action="{{route('admin.categories.update',$row->id)}}" enctype="multipart/form-data">
                                    @csrf
                                    @method('PUT')
                                    <div class="form-group">
                                    ...
                                    <div class="form-group">
                                      <label>القسم الاساسي
                                        (<small class="text-info">أختياري</small>)
                                      </label>
                                      <select class="form-control" name="parent_id">
                                        <option value="">أختر القسم</option>
                                        @foreach($categories->where('id','!=',$row->id) as $section)
                                        <option
                                        {{$row->parent_id == $section->id ? 'selected' : ''}}
                                        value="{{$section->id}}"
